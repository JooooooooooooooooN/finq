export async function onRequest(context) {
  ...
  try {
    ...
    // query2 우선, 실패 시 query1 재시도
    let res = await fetch(`https://query2.finance.yahoo.com/v10/finance/quoteSummary/${encoded}?modules=${modules}`, { headers });
    if (!res.ok) {
      res = await fetch(`https://query1.finance.yahoo.com/v10/finance/quoteSummary/${encoded}?modules=${modules}`, { headers });
    }
